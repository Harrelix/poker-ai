    ...
    pub fn next_betting_round(&mut self) {
        ...
        match self.betting_round {
            ...
            BettingRound::River => {
                ...
                let community_array: [Card; 5] = self
                    .community
                    .clone()
                    .try_into()
                    .unwrap_or_else(|v: Vec<Card>| panic!("Community not full ({}/5)", v.len()));
                // find best hand of all players
                let winning_hands: Vec<Hand> = self
                    .players
                    .iter()
                    .map(|player| {
                        Hand::get_all_hands(concat_arrays!(player.hole, community_array))
                            .into_iter()
                            .max()
                            .unwrap()
                    })
                ...
                for index in winning_players_indices.iter() {
                    self.players[*index].stack += self.pot_size / winning_players_indices.len();
                }
                println!(
                    "Winner: {}",
